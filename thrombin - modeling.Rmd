...
conf_mtrx <- function(x, dfm = NULL, mdl = NULL, smpl = NULL, means = FALSE, emp = FALSE) {
  ...
  #         dfm:  optional dataframe to append the results to.
  ...
  #         means:optional value to force the calculation of mean for all data.
  #         emp:  optional value to add an empty line to the end of data frame.
  ...
  x <- tryCatch({
    ...
    # accuracy based on the class distribution and imbalance
...
thrombin_new <- cbind(thrombin.reduced[1], thrombin_pca$x[,1:var.cnt])
barplot(pca.var[1:100], xlab = "Top 100 Principal Components", ylab = "Percent of Variance")
#write.csv(thrombin_new, file = "thrombin_reduced.csv", row.names = FALSE)
...
set.seed(101)
idx_trainset <- sample(nrow(thrombin_new), floor(nrow(thrombin_new) * 0.70))
thrombin.train <- thrombin_new[idx_trainset,] 
thrombin.test <- thrombin_new[-idx_trainset,] 
#
...
#
Active.cnt <- length(which(thrombin_new[,1] == "A"))
Inactive.cnt <- nrow(thrombin_new) - Active.cnt
...
## Further Analysis for Selected Models (10 fold cross-validation)
```{r}
...
folds <- createFolds(thrombin_new$V1, 10)
i <- 0
for (f in folds) { 
  i <- i + 1
  # build the training and testsets based on each fold
  thrombin.train <- thrombin_new[-f,] 
  thrombin.test <- thrombin_new[f,]
  ...
  if (i == 1){
    ...
    test10 = thrombin.test
    both10 <- smote.both
    under10 <- smote.under
    over10 <- smote.over
  }
}
thrombin.testset <- list(test1, test2, test3, test4, test5, test6, test7, test8, test9, test10)
thrombin.sample.bal <- list(both1, both2, both3, both4, both5, both6, both7, both8, both9, both10)
thrombin.sample.under <- list(under1, under2, under3, under4, under5, under6, under7, under8, under9, under10)
thrombin.sample.over <- list(over1, over2, over3, over4, over5, over6, over7, over8, over9, over10)

#
# Decision Tree using SMOTE balanced sampling
results <- data.frame()
i <- 0
for (i in 1:10) {
  ...
  results <- conf_mtrx(thrombin.table, dfm = results, mdl = "DT", smpl = c("smote-o/u", nrow(thrombin.train)), means = (i == 10), emp = (i == 10))
}
...
for (i in 1:10) {
  ...
  thrombin.pred <- predict(thrombin.model, newdata = thrombin.test, type = thrombin.model$method)
  thrombin.table <- table(thrombin.pred, thrombin.test$V1)
  results <- conf_mtrx(thrombin.table, dfm = results, mdl = "DT", smpl = c("smote-under", nrow(thrombin.train)), means = (i == 10), emp = (i == 10))
}
all.results <- rbind(all.results, results)
#
...
for (i in 1:10) {
  ...
  thrombin.pred <- predict(thrombin.model, newdata = thrombin.test, type = thrombin.model$method)
  thrombin.table <- table(thrombin.pred, thrombin.test$V1)
  results <- conf_mtrx(thrombin.table, dfm = results, mdl = "J48", smpl = c("smote-o/u", nrow(thrombin.train)), means = (i == 10), emp = (i == 10))
}
all.results <- rbind(all.results, results)
#
# C4.5 Decision Tree (J48) using SMOTE under sampling
results <- data.frame()
i <- 0
for (i in 1:10) {
  thrombin.train <- thrombin.sample.under[[i]]
  ...
  thrombin.pred <- predict(thrombin.model, newdata = thrombin.test, type = thrombin.model$method)
  thrombin.table <- table(thrombin.pred, thrombin.test$V1)
  results <- conf_mtrx(thrombin.table, dfm = results, mdl = "J48", smpl = c("smote-under", nrow(thrombin.train)), means = (i == 10), emp = (i == 10))
}
all.results <- rbind(all.results, results)
#
...
for (i in 1:10) {
  ...
  results <- conf_mtrx(thrombin.table, dfm = results, mdl = "SSVM", smpl = c("smote-over", nrow(thrombin.train)), means = (i == 10), emp = (i == 10))
}
all.results <- rbind(all.results, results)
#
...
for (i in 1:10) {
  ...
  results <- conf_mtrx(thrombin.pred$confusion, dfm = results, mdl = "HDDA", smpl = c("smote-o/u", nrow(thrombin.train)), means = (i == 10), emp = (i == 10))
}
all.results <- rbind(all.results, results)
#
...
for (i in 1:10) {
  ...
  thrombin.model <- hdda(thrombin.train[-1], thrombin.train$V1, show = FALSE, d_select = "Cattell", graph = FALSE, threshold = 0.05)
  thrombin.pred <- predict(thrombin.model, thrombin.test[-1], thrombin.test$V1)
  results <- conf_mtrx(thrombin.pred$confusion, dfm = results, mdl = "HDDA", smpl = c("smote-under", nrow(thrombin.train)), means = (i == 10), emp = (i == 10))
}
all.results <- rbind(all.results, results)
#
# Add two HDDA results with internal 10-fold CV (for comparison purposes)
results <- conf_mtrx(thrombin.smote_b.hdda.pred$confusion, mdl = "HDDA", smpl = c("Int/smote-o/u", nrow(thrombin.balanced.smote_b)))
all.results <- rbind(all.results, results)
results <- conf_mtrx(thrombin.smote_u.hdda.pred$confusion, mdl = "HDDA", smpl = c("Int/smote-under", nrow(thrombin.balanced.smote_u)))
all.results <- rbind(all.results, results)
all.results
write.csv(all.results, file = "final-results.csv", row.names = FALSE, quote = FALSE, sep = ",")
paste("Total time to execute the RF model:", elapsed_time(start_time))
```
